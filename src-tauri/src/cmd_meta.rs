...
pub fn start_clock_thread(app: tauri::AppHandle, config: Arc<Mutex<AppConfig>>) {
    thread::spawn(move || {
        loop {
            ...
            let _ = app.emit("clock-tick", ClockTickPayload { time_str, date_str, clock_24h });
...
fn format_date(lang: &str, weekday: usize, day: usize, month0: usize, year: u32) -> String {
    const DAYS_ES:   [&str; 7] = ["Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"];
    const MONTHS_ES: [&str;12] = ["Enero","Febrero","Marzo","Abril","Mayo","Junio",
                                   "Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];
    const DAYS_EN:   [&str; 7] = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];
    const MONTHS_EN: [&str;12] = ["January","February","March","April","May","June",
                                   "July","August","September","October","November","December"];
    if lang == "en" {
        format!("{}, {} {}, {}", DAYS_EN[weekday.min(6)], MONTHS_EN[month0.min(11)], day, year)
    } else {
        format!("{}, {} de {} {}", DAYS_ES[weekday.min(6)], day, MONTHS_ES[month0.min(11)], year)
    }
}
