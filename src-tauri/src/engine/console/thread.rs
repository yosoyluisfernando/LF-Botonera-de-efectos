...
pub fn run(rx: Receiver<ConsoleCommand>, state: Arc<Mutex<ConsoleState>>) {
    ...
    for cmd in rx {
        match cmd {
            ConsoleCommand::SetBusRouting { bus, routing } => {
                let same = state
                    .lock()
                    .unwrap()
                    .slots
                ...
                // Reconstruir corta lo que suene, asi que no se hace por gusto:
                // reaplicar el mismo ruteo (al arrancar, al reconectar) no debe
                // callar nada. Que el bus no exista si obliga: hay que abrirlo.
