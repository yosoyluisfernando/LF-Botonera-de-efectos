...
#[derive(Deserialize)]
struct LegacyButton {
    id: String, index: u32, label: String, path: String,
    color_bg: String, color_text: String,
...
pub fn load_config() -> AppConfig {
    ...
    let raw  = match fs::read_to_string(&path) { Ok(s) => s, Err(_) => return AppConfig::default() };
...
fn migrate(old: LegacyConfig) -> AppConfig {
    ...
    if let Some(v) = old.is_first_boot          { cfg.is_first_boot = v; }
    if let Some(v) = old.weather_module_enabled  { cfg.weather_module_enabled = v; }
    if let Some(v) = old.lf_automatizador_link   { cfg.lf_automatizador_link = v; }
    if let Some(v) = old.theme                   { cfg.theme = v; }

    if let Some(profile) = cfg.profiles.first_mut() {
        if let Some(dev) = old.audio_device { profile.audio.out_main = dev; }
        if let Some(paleta) = profile.paletas.first_mut() {
            if let Some(grid) = load_legacy_grid() {
                ...
                paleta.botones = grid.buttons.unwrap_or_default()
                    .into_iter()
                    .map(|b| ButtonData {
                        id: b.id, index: b.index, label: b.label.clone(),
                        type_field: "audio".to_string(), path: b.path,
                        folder: String::new(), name: b.label,
                        color_bg: b.color_bg, color_text: b.color_text,
                        vol: 1.0, duration: 0.0,
    ...
    for profile in cfg.profiles.iter_mut() {
        for paleta in profile.paletas.iter_mut() {
            ...
            for b in paleta.botones.iter_mut() {
                ...
                if b.id != expected { b.id = expected; }
            }
        }
    }
}
