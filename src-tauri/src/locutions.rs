...
pub fn resolve_climate_file(folder: &str, kind: &str, value: f64) -> Result<String, String> {
    ...
    find_prefix(&entries, &prefix, None)
        .ok_or(format!("No hay locución para el valor {}", rounded))
}
...
fn read_folder(folder: &str) -> Result<Vec<PathBuf>, String> {
    ...
    Ok(fs::read_dir(folder).map_err(|e| e.to_string())?
        .filter_map(|e| e.ok())
        .map(|e| e.path())
        .collect())
}
...
fn find_prefix(entries: &[PathBuf], prefix: &str, exclude: Option<&str>) -> Option<String> {
    ...
    entries.iter().find(|p| {
        ...
        name.starts_with(&prefix)
            && exclude.map_or(true, |ex| !name.contains(ex))
