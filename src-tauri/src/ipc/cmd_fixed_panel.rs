...
#[derive(Serialize)]
pub struct FixedPanelState {
    pub settings: FixedPanelConfig,
...
pub(super) fn state(cfg: &AppConfig) -> FixedPanelState {
    ...
    FixedPanelState {
        settings,
        buttons: buttons(cfg).iter().map(button_to_view).collect(),
    }
}
...
pub fn set_fixed_panel_settings(
    ...
    cfg.fixed_panel.scope = scope;
    cfg.fixed_panel.view = view;
    cfg.fixed_panel.side = side;
    cfg.fixed_panel.visible = visible;
    cfg.fixed_panel.show_on_start = show_on_start;
    cfg.fixed_panel.columns = columns;
    cfg.fixed_panel.row_mode = row_mode;
    cfg.fixed_panel.rows = rows;
    cfg.fixed_panel.width = width;
...
pub(super) fn ensure_capacity(cfg: &AppConfig, replacing: bool) -> Result<(), String> {
    if !replacing
        && cfg.fixed_panel.row_mode == "limited"
...
pub fn clear_fixed_scope(scope: String, state_: tauri::State<AppState>) -> Result<(), String> {
    ...
    match scope.as_str() {
        ...
        "profile" => cfg
            .profiles
            .iter_mut()
            .for_each(|p| p.fixed_buttons.clear()),
...
pub fn reorder_fixed_buttons(
    ...
    list[source].index = to_index;
    list[source].id = mappings[0].1.clone();
    if let Some(target) = target {
        mappings.push((
            list[target].id.clone(),
            format!("{prefix}_btn_{from_index}"),
        ));
        list[target].index = from_index;
        list[target].id = mappings[1].1.clone();
    }
    config_io::save_config(&cfg)?;
    let next = state(&cfg);
    drop(cfg);
